...
const UpdateTrip = () => {
  ...
  const onSubmit: SubmitHandler<PutTripForm> = async (data) => {
    try {
      if (trip.id !== undefined) {
        await putTrip(trip.id, data); // Truyền id của trip vào hàm putTrip
        navi("/admin/trips");
      } else {
        console.error("ID của trip không tồn tại");
      }
    } catch {
